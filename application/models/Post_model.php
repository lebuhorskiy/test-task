...
class Post_model extends CI_Emerald_Model
{
    ...
    public function get_likes() {
       ...
       $likes = Likes_model::getAllById($this->get_id(), $this->type);
    ...
    private static function _preparation_main_page($data)
    {
        ...
        foreach ($data as $d){
            ...
            $o->isLiked = Likes_model::getByPostIdAndUser($d->get_id(),'post', 1);
            $o->time_created = $d->get_time_created();
            $o->time_updated = $d->get_time_updated();
            $o->likes = Likes_model::getByPostId($d->get_id(), 'post');
            $o->comments = Comment_model::get_all_by_assign_id($d->get_id());
            $ret[] = $o;
        }
